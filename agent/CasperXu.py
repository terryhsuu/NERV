...
class BaseAgent():
    def __init__(self, color="black", rows_n=8, cols_n=8, width=600, height=600):
        ...
        self.col_offset = (width - height)/2 + 0.1 * \
            min(height, width) + 0.5 * self.block_len
    ...
    def step(self, reward, obs):
        ...
        raise NotImplementError(
            "You didn't finish your step function. Please override step function of BaseAgent!")
...
class MyAgent(BaseAgent):
        ...
        for i, row in enumerate(self.rows):
            for j, col in enumerate(self.cols):
                x = 0.1 * self.side_length + 0.8 * \
                    (j+0.5) / len(self.cols) * self.side_length
                y = 0.1 * self.side_length + 0.8 * \
                    (i+0.5) / len(self.rows) * self.side_length
    ...
    def how_close_to_edge(self, label):
        edge = {0, 1, 2, 3, 4, 5, 6, 7, 8, 16, 24, 32, 40, 48, 56,
                15, 23, 31, 39, 47, 55, 57, 58, 59, 60, 61, 62, 63}
        degree = 63
        for i in edge:
    ...
    def if_give_corner(self, label):
        ...
        pos = self.enum[label]
        for i in [0, 7, 56, 63]:
            if abs(i-pos) == 1 and status[i] != self.cur_player:
                return False
        return True
    ...
    def step(self, reward, obs):
        ...
        if self.endpoint():
            ...
            if self.if_corner(avail_step2) != -1:
                return (action_dict[self.rev_enum[self.if_corner(avail_step2)]], pygame.USEREVENT)
            amount = {label: self.eat_amount(
                label) for label in avail_step if self.if_risk(label)}
